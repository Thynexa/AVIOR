...
>
> **格式说明**：avior 生成的 YAML 无注释；block / flow 风格逐产物固定（PRD FR-X-8 规范化序列化，以 PRD §6 与本示例的结构为准，如 inventory 包行与 scores 的 metrics 行为 flow map）。本示例文件中的行内注释与对齐空白为**讲解用途**，字段名、结构与取值与冻结契约逐项一致。
