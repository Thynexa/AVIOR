...
| CLI / R 包名 | `avior` |
| 文档版本 | v1.0（开发基线；本文件取代《ValiR PRD.md》v0.1，后者保留作历史参考） |
...
| FR-DRAFT-3 | 数据边界：显式 opt-in；模型端点可配置（含本地模型）；默认只发送公开包的元数据与签名，客户私有代码永不外发除非显式配置 | P1 |
...
└── validation/
    ...
    └── evidence/              # 不可变证据包（是否入 git 由团队决定，建议入）
        └── bundle-<timestamp>/
```
...
| 开源许可 | Apache-2.0（建议，待定 §11） | 专利条款对企业采纳友好 |
...
| 决策点 | W12 | 依试点反馈决定 V1.1 优先级 | AI 起草（FR-DRAFT，需先答复 §11-Q1） vs 自研包线提前 |
...
| # | 问题 | 需在何时回答 |
| --- | --- | --- |
| Q1 | AI 起草的隐私边界：第三方大模型 vs 本地模型的默认选择；客户源码外发的红线条款 | V1.1 开工前（FR-DRAFT-3 依赖） |
| Q2 | 开源许可最终确认（建议 Apache-2.0） | 首次公开发布前 |
| Q3 | `evidence/` 是否默认纳入 git（仓库体积 vs 证据集中度） | M0 结合 design partner 仓库规范定 |
| Q4 | 英文报告字符串是否提前到 V1（若 design partner 有国际申报需求） | M0 |
